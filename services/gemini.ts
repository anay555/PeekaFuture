...
export const generateMarketInsights = async (career: string): Promise<MarketInsightAnalysis> => {
    ...
    const prompt = `
        ...
        Analyze the current job market for an entry-level "${career}". Use Google Search to get up-to-date information.

        Provide your analysis as a single, clean JSON object. Do not include any text, explanation, or markdown formatting before or after the JSON object.
